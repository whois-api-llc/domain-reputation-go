...
const (
	...
	userAgent      = "domain-reputation-go/" + libraryVersion
	mediaType      = "application/json"
)

// defaultDomainReputationURL is the default Domain Reputation API URL.
const defaultDomainReputationURL = `https://domain-reputation.whoisxmlapi.com/api/v2`
...
func NewClient(apiKey string, params ClientParams) *Client {
	...
	apiBaseURL := params.DomainReputationBaseURL
	if apiBaseURL == nil {
		apiBaseURL, err = url.Parse(defaultDomainReputationURL)
		if err != nil {
			panic(err)
		}
	}
	...
	client.DomainReputationService = &domainReputationServiceOp{client: client, baseURL: apiBaseURL}
